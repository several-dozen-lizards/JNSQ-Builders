...
export function installFirstPerson({camera,controls,canvas,world,button,help,invalidate,onEnter=()=>{}}){
  const doc=canvas.ownerDocument,win=doc.defaultView,keys=new Set(),direction=new THREE.Vector3();
  ...
  const hint='WASD to walk · Shift to move faster · Click for mouse look (or drag) · Esc to return. Objects can be walked through.';
  ...
  function clear(){keys.clear();drag=null;}
  function exit(){
  ...
  listen(doc,'pointerdown',e=>{
    if(!active)return;
    if(e.target!==canvas){
      if(e.target.closest?.('#workshopRibbon [role="tab"]')){clear();return;}
      if(!button.contains(e.target))exit();return;
    }
  ...
  for(const type of ['pointerup','pointercancel'])listen(doc,type,e=>{if(active&&(e.target===canvas||drag)){drag=null;stop(e);}},true);
  for(const type of ['click','dblclick','contextmenu','wheel'])listen(doc,type,e=>{if(active&&e.target===canvas)stop(e);},{capture:true,passive:false});
  listen(doc,'keydown',e=>{
    if(!active)return;
    if(e.code==='Escape'||e.key==='Escape'){stop(e);exit();return;}
  ...
  listen(doc,'pointerlockchange',()=>{if(doc.pointerLockElement===canvas){if(!active){doc.exitPointerLock?.();return;}hadLock=true;}else if(active&&hadLock)exit();});
