...
import {eyeAnchor} from './first-person.mjs';
...
export async function installVisitor({scene,camera,controls,canvas,world,restore,invalidate,status,arrival,id,applyEnvironment,onReady,onError}){
  ...
  const furniture=furnitureLayer({scene,invalidate,status,ground:(x,z)=>heightAt(world,x,z)});
  ...
  const ground=(x,z,y)=>{footRay.set(new THREE.Vector3(x,y+.45,z),down);footRay.far=.9;
    const hit=footRay.intersectObjects(scene.children,true).find(h=>h.object.isMesh&&!h.object.material?.transparent&&!h.object.userData.visitorBody&&!h.object.userData.walkThrough&&h.face?.normal.y>.5);return hit?hit.point.y:null;};
  ...
  function focus(){const rec=snapshot?.members[member];if(!rec)return;const p=pose(rec);
    if(firstPerson){camera.position.copy(eyePosition(p));camera.rotation.order='YXZ';camera.rotation.set(pitch,yaw,0);}
  ...
  async function body(name,rec){
    const root=new THREE.Group();scene.add(root);const marker=new THREE.Mesh(new THREE.CapsuleGeometry(.22,1.15,6,10),new THREE.MeshStandardMaterial({color:name===member?0xffce80:0x80d6b6}));marker.position.y=.8;root.add(marker);
    ...
    try{
      ...
      const gltf=await loader.loadAsync(url);if(bodies.get(name)!==entry)return;
      ...
      const bounds=new THREE.Box3().setFromObject(gltf.scene);gltf.scene.position.y-=bounds.min.y;
      ...
      const clip=await clipPromise;if(!entry.walk&&clip)entry.gait=createGait(gltf.scene,root,clip,ground);
      invalidate();
    }catch{status(name+': avatar unavailable; showing a body marker.',true);}
  }
  ...
  async function walk(x,z,control){if(!connected)return true;try{const r=await act('walk',{to:[x,-z],control,expected_room:rid});status(r.blocked_by?'Stopped at '+r.blocked_by.replaceAll('_',' '):control==='run'?'Running':'Walking');await refresh();return !!r.blocked_by;}catch(e){status(e.message,true);return true;}}
  canvas.addEventListener('dblclick',e=>{e.preventDefault();e.stopImmediatePropagation();const rect=canvas.getBoundingClientRect(),ray=new THREE.Raycaster();ray.setFromCamera(new THREE.Vector2((e.clientX-rect.left)/rect.width*2-1,-(e.clientY-rect.top)/rect.height*2+1),camera);const hits=ray.intersectObjects(scene.children,true).filter(h=>h.object.isMesh&&!h.object.material?.transparent&&!h.object.userData.visitorBody&&!h.object.userData.walkThrough);const p=hits[0]?.point;if(p)walk(p.x,p.z);},true);
  let dragging=null;
  ...
  canvas.addEventListener('pointermove',e=>{if(firstPerson){if(dragging&&document.pointerLockElement!==canvas){yaw-=(e.clientX-dragging[0])*.004;pitch=Math.max(-1.35,Math.min(1.35,pitch-(e.clientY-dragging[1])*.004));dragging=[e.clientX,e.clientY];focus();}e.stopImmediatePropagation();}},true);
  document.addEventListener('mousemove',e=>{if(firstPerson&&document.pointerLockElement===canvas){yaw-=e.movementX*.004;pitch=Math.max(-1.35,Math.min(1.35,pitch-e.movementY*.004));focus();}});
  canvas.addEventListener('pointerup',e=>{dragging=null;if(firstPerson)e.stopImmediatePropagation();},true);
  const walkInput=createWalkInput({motionEnd:()=>{const m=snapshot?.members[member]?.movement;return m?m.started_at+m.total_duration_s:0;},step:async (direction,running)=>{const rec=snapshot?.members[member];if(!rec)return true;const p=pose(rec);return walk(p.x+direction.x*.9,p.z+direction.z*.9,running?'run':'walk');}});
  canvas.tabIndex=0;
  ...
  function render(now){if(stopped)return;requestAnimationFrame(render);walkInput.tick(yaw);const delta=(now-previous)/1000;previous=now;let moving=false;
    for(const [name,entry] of bodies){const p=pose(entry.rec),motion=entry.rec.movement,isMoving=motion&&Date.now()/1000<motion.started_at+motion.total_duration_s;entry.root.position.copy(p);entry.root.rotation.y=(entry.rec.heading_deg||0)*Math.PI/180;entry.root.visible=!(firstPerson&&name===member);entry.root.traverse(o=>o.userData.visitorBody=true);if(entry.walk){entry.walk.paused=!isMoving;entry.mixer.update(delta);}if(entry.root.visible)entry.gait?.update(isMoving);moving||=!!isMoving;}
    const me=snapshot?.members[member];if(me){const p=pose(me);label.textContent=`${member} · elevation ${p.y.toFixed(2)} m`;const near=nearbyTeleporters(snapshot,p);returnButton.textContent=near.length?'E · '+snapshot.objects[near[0]].name+' — destinations':'';if(firstPerson){camera.position.copy(eyePosition(p));camera.rotation.set(pitch,yaw,0);}if(moving||firstPerson)invalidate();}
  }
